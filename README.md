...
```text
/order 1
/edit_order 1 добавь в работы диагностику 1500
/delete_order 1
```

`/edit_order` теперь понимает и точечные изменения заказа. В свободной форме можно добавить, изменить или удалить отдельную позицию без полного переписывания заказа.

Для удобства сначала посмотрите текущий заказ:

```text
/order 1
```

В ответе позиции будут пронумерованы, например:

```text
Заказ #1 | 2026-04-21 11:00-14:00
Позиции:
1. Работы: замена масла (1 x 2 500 RUB = 2 500 RUB)
2. Запчасти: фильтр (1 x 700 RUB = 700 RUB)
3. Запчасти: масло (1 x 2 400 RUB = 2 400 RUB)
4. Расходники: очиститель (1 x 200 RUB = 200 RUB)
```

Пример: добавить новую позицию:
...
```text
В заказе 1 добавь в работы диагностику 1500
```

Пример: изменить существующую позицию по номеру:

```text
/edit_order 1 измени позицию 3 на масло 2600
```

или в свободной форме:

```text
В заказе 1 измени позицию 3 на масло 2600
```

Пример: изменить существующую позицию по названию:

```text
В заказе 1 измени масло на 2600
```

Пример: удалить позицию из существующего заказа:

```text
/edit_order 1 удали позицию 4
```

или в свободной форме:

```text
В заказе 1 удали очиститель
```

Если нужно, бот по-прежнему может принять и полную замену заказа целиком:

```text
/edit_order 1 Иван Петров, +79991234567, Toyota Camry А123ВС77. 2026-04-21 11:00-14:00. Работы: замена масла 3000, диагностика 1500. Запчасти: фильтр 700, масло 2600. Расходники: очиститель 200.
```

Пример: удалить весь заказ в свободной форме:
